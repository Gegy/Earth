...
public class ConformingDelaunayTriangulator {
    ...
    private Coordinate[] getPointArray() {
        Coordinate[] pts = new Coordinate[this.initialVertices.size()
                + this.segVertices.size()];
        int index = 0;
        for (Object initialVertice : initialVertices) {
            Vertex v = (Vertex) initialVertice;
            pts[index++] = v.getCoordinate();
        }
        for (Object segVertice : segVertices) {
            Vertex v = (Vertex) segVertice;
            pts[index++] = v.getCoordinate();
        }
        return pts;
    }
    ...
    private void insertSites(Collection vertices) {
        ...
        for (Object vertice : vertices) {
            ConstraintVertex v = (ConstraintVertex) vertice;
            this.insertSite(v);
        }
    }
    ...
    public void enforceConstraints() {
        ...
        do {
            ...
            Debug.println("Iter: " + count + "   Splits: " + splits
                    + "   Current # segments = " + this.segments.size());
    ...
    private int enforceGabriel(Collection segsToInsert) {
         ...
         */
        for (Object aSegsToInsert : segsToInsert) {
            Segment seg = (Segment) aSegsToInsert;
