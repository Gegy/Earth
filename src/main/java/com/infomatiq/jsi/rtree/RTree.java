...
public class RTree implements SpatialIndex {
    private static final Logger log = LogManager.getLogger(RTree.class);
    private static final Logger deleteLog = LogManager.getLogger(RTree.class
            .getName() + "-delete");
    ...
    private void createNearestNDistanceQueue(Point p, int count,
        ...
        this.distanceQueue.setSortOrder(PriorityQueue.SORT_ORDER_DESCENDING);
                ...
                for (int i = 0; i < n.entryCount; i++) {
                    ...
                    if (entryDistanceSq <= furthestDistanceSq) {
                        ...
                        if (this.savedValues.size() > 0
                                && this.savedPriority == this.distanceQueue.getPriority()) {
                        ...
                        if (this.distanceQueue.getPriority() < furthestDistanceSq
                                && this.distanceQueue.size() >= count) {
    ...
    public void nearestN(Point p, TIntProcedure v, int count,
        ...
        this.distanceQueue.setSortOrder(PriorityQueue.SORT_ORDER_ASCENDING);
    ...
    private Node splitNode(Node n, float newRectMinX, float newRectMinY,
        ...
        this.pickSeeds(n, newRectMinX, newRectMinY, newRectMaxX, newRectMaxY, newId,
        ...
        if (INTERNAL_CONSISTENCY_CHECKING) {
            ...
            if (!nMBR.equals(this.calculateMBR(n))) {
            ...
            if (!newNodeMBR.equals(this.calculateMBR(newNode))) {
        ...
        while (n.level != this.treeHeight) {
            ...
            if (nn != null) {
                if (parent.entryCount < this.maxNodeEntries) {
                    ...
                    newNode = this.splitNode(parent, nn.mbrMinX, nn.mbrMinY, nn.mbrMaxX,
    ...
    private boolean checkConsistency(int nodeId, int expectedLevel,
        ...
        if (nodeId == this.rootNodeId && this.size() == 0) {
            if (n.level != 1) {
        ...
        for (int i = 0; i < n.entryCount; i++) {
            ...
            if (n.level > 1) { // if not a leaf
                if (!this.checkConsistency(n.ids[i], n.level - 1, new Rectangle(
                        n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i],
                        n.entriesMaxY[i]))) {
                    return false;
                }
