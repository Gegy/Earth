...
public class ShapeWriter {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (this.decimationDistance > 0.0) {
                boolean isDecimated = prev != null
                        && Math.abs(coords[i].x - prev.x) < this.decimationDistance
                        && Math.abs(coords[i].y - prev.y) < this.decimationDistance;
                if (i < n && isDecimated) {
                    continue;
                }
                prev = coords[i];
    ...
    private GeneralPath toShape(LineString lineString) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (this.decimationDistance > 0.0) {
                boolean isDecimated = prev != null
                        && Math.abs(currentCoord.x - prev.x) < this.decimationDistance
                        && Math.abs(currentCoord.y - prev.y) < this.decimationDistance;
                if (i < n && isDecimated) {
                    continue;
                }
                prev = currentCoord;
